...
Previously `inclusion: manual`, which meant the release safety rules under
"Publishing is irreversible" loaded only if someone explicitly referenced
`#git-and-releasing`. That is the least recoverable operation here, so it should
never have depended on being asked for.
